...
save_witnesses(Height) ->
    gen_server:call(?MODULE, {save_witnesses, Height}, infinity).
...
handle_call(check_witness_refresh, _From, State) ->
    ...
    {reply, Check, State};
handle_call({save_witnesses, Height}, _From, #state{miner=Miner}=State) ->
    ...
    ok = lists:foreach(fun({PubkeyBin, GW}) ->
                               Key = libp2p_crypto:bin_to_b58(PubkeyBin),
                               Witnesses = [libp2p_crypto:bin_to_b58(A) || A <- maps:keys(blockchain_ledger_gateway_v2:witnesses(GW))],
                               ...
                               case ets:member(witness_ets, Key) of
                                   ...
                                   true ->
                                       ...
                                       true = ets:insert(witness_ets, {Key, lists:usort(lists:flatten([Val | TaggedWitnessList]))})
                               end
                       end,
                       maps:to_list(AG)),
